...
init_per_suite(Config) ->
    ...
    case net_kernel:start([lmdb_test, shortnames]) of
        {ok, _} -> ok;
        {error, _} -> ok
    end,
    cover:start(),
...
insert_delete_test(Config) ->
    ...
    {ok, Refs} = vmq_lmdb_store:msg_store_find({"", "foo"}),
    %% delete all
    ok = delete_msgs({"", "foo"}, Msgs),
    {ok, []} = vmq_lmdb_store:msg_store_find({"", "foo"}),
...
store_msgs(SId, [Msg|Rest]) ->
    ok = vmq_lmdb_store:msg_store_write(SId, Msg),
    store_msgs(SId, Rest);
store_msgs(_, []) -> ok.

delete_msgs(_, []) -> ok;
delete_msgs(SId, [#vmq_msg{msg_ref=Ref}|Rest]) ->
    ok = vmq_lmdb_store:msg_store_delete(SId, Ref),
...
read_msgs(SId, [Ref|Refs], Acc) ->
    {ok, Msg} = vmq_lmdb_store:msg_store_read(SId, Ref),
    read_msgs(SId, Refs, [Msg|Acc]).

refcount([Ref|Refs], Cnt) ->
    refcount(Refs, Cnt + vmq_lmdb_store:refcount(Ref));
refcount([], Cnt) -> Cnt.
...
store_summary() ->
    vmq_lmdb_store_utils:full_table_scan(
      fun
