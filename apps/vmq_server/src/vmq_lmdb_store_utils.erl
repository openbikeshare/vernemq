...
full_table_scan(_,Acc) ->
    %% XXX-TODO reimplement this with lmdb
    Acc.

%% full_table_scan(FoldFun, Acc) ->
%%     full_table_scan_(vmq_lmdb_store_sup:get_bucket_pids(), {FoldFun, Acc}).

%% full_table_scan_([Bucket|Rest], Acc) ->
%%     FoldOpts = [{fill_cache, false}],
%%     NewAcc = eleveldb:fold(vmq_lmdb_store:get_ref(Bucket), fun full_table_scan__/2, Acc, FoldOpts),
